...
function readAuthMode() {
  const raw = readSetting('authMode', 'player-and-admin');
  return raw === 'admin-only' ? 'admin-only' : 'player-and-admin';
}
...
function requirePlayerToken(req, res, next) {
  if (readAuthMode() === 'admin-only') return next();
...
app.get('/api/auth-mode', (_req, res) => {
  res.json({ mode: readAuthMode() });
});

app.put('/api/auth-mode', (req, res) => {
  const mode = req.body?.mode === 'admin-only' ? 'admin-only' : 'player-and-admin';
  writeSetting('authMode', mode);
